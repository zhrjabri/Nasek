/*
 * A DOM for the booking harness.
 *
 * `verify-booking.tsx` mounts the real booking page and the real customer
 * dashboard, with the real store, and reads the anchors back out of them — the
 * question it exists to answer is whether a WhatsApp link is present, and when.
 * That needs a document before React is ever loaded, so this file builds the
 * window, publishes the globals React and the harness read off `globalThis`,
 * and only then imports the built bundle.
 *
 * The origin is the customer site rather than the owner portal, because that is
 * where a booking is made.
 */
...
await import('../.verify/verify-booking.js')
